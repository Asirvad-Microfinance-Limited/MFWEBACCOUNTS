...
var BalanceRecoveryApprove = {
    ...
    checkAccess: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/PayratQueries", CheckAccess, BalanceRecoveryApprove.checkAccessRtn, userdata.token)
    },
        ...
        if (response.status === "SUCCESS") {
            ...
            if (response.data.queryResult.QueryResult.length > 0) {
                ...
                if (x == "0") {
                    ...
                    BalanceRecoveryApprove.BranchLoad();
                }
    ...
    tokenValidate: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/UserSession", CheckToken, BalanceRecoveryApprove.checkAccessToken, userdata.token)
    },
    ...
    BranchLoad: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/BalanceRecovery", GetBranchLoad, BalanceRecoveryApprove.FillBranchLoad, userdata.token)

    },
    FillBranchLoad: function (response) {
        jQuery('.page-loader-wrapper').hide();
        if (response.status === "SUCCESS") {
            response.data.queryResult = JSON.parse(DecryptAPIReq(response.data.encryptedResStr));
    ...
    EmployeeLoad: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/BalanceRecovery", GetEmployeeLoad, BalanceRecoveryApprove.FillEmployeeLoad, userdata.token)
    ...
    tablefill: function (emcode, brid) {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/BalanceRecovery", brnachtablefill, BalanceRecoveryApprove.branchtableload, userdata.token)
    },
            ...
            if (response.data != null && response.data.queryResult.tableFill.length > 0) {
                ...
                if (response.data.queryResult.tableFill.length > 0) {
                    ...
                    jQuery.each(response.data.queryResult.tableFill, function (i, val) {
                        ...
                        var data = val.empName;
                        var postid = data.split("~");
                        var post = postid[1];
                        var dep = postid[2];
                        var empname = postid[0];
                        ...
                        $row.append(jQuery('<td class="HCol" align="center">').html(empname));
                        $row.append(jQuery('<td class="HCol" align="center">').html(dep));
                        $row.append(jQuery('<td class="HCol" align="center">').html(post));
    ...
    Staffrecoveryapproval: function (result) {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/BalanceRecovery", AddButton, BalanceRecoveryApprove.Staffrecoveryapprovalload, userdata.token)
    },
...
jQuery(document).ready(function ($) {
    ...
    BalanceRecoveryApprove.tokenValidate();
    BalanceRecoveryApprove.checkAccess();
    //BalanceRecovery.BranchLoad();

    jQuery('#Branchid').change(function (e) {
        BalanceRecoveryApprove.EmployeeLoad();
        jQuery('#maincard').hide();
    ...
    jQuery('#empid').change(function (e) {
        ...
        var brid = jQuery('#Branchid').val();
        BalanceRecoveryApprove.tablefill(emcode, brid);
    ...
    jQuery('#btnSaveBank').on('click', function () {
        ...
        BalanceRecoveryApprove.Staffrecoveryapproval(dataString);
        //    }
    });
