var FDIntAccrualAuto = {
    ...
    checkAccess: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/PayratQueries", CheckAccess, FDIntAccrualAuto.checkAccessRtn, token)
    },
    ...
    tokenValidate: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/UserSession", CheckToken, FDIntAccrualAuto.checkAccessToken, userdata.token)
    },
    ...
    loaddata: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectDataQueries", loandata, FDIntAccrualAuto.FillTable, userdata.token);
    ...
    //        FDIntAccrualAuto.FillTable(Response);
    ...
    submitdata1: function () {
        ...
        var table = document.getElementById('tbldespatch');
        var rowLength = table.rows.length;
        //for (i = 1; i < $("#example tr").length; i++) {
        //    //var dateTime = table.rows[i].cells[7].innerText;
        //    //var date1 = dateTime.slice(0, 10);

        //    //var dateTime = table.rows[i].cells[8].innerText;
        //    //var date2 = dateTime.slice(0, 10);

        //    tblAmntData = tblAmntData + table.rows[i].cells[0].innerText + "µ" + table.rows[i].cells[1].innerText + "µ" + table.rows[i].cells[2].innerText; + "µ" + table.rows[i].cells[3].innerText + "µ" + table.rows[i].cells[4].innerText + "µ" + table.rows[i].cells[5].innerText + "µ" + table.rows[i].cells[6].innerText + "µ" + table.rows[i].cells[7].innerText + "µ" + table.rows[i].cells[8].innerText + "¥";
        //}

        for (i = 1; i < rowLength; i++) {


           var tblAmntData = tblAmntData + table.rows[i].cells[0].innerText + "µ" + table.rows[i].cells[1].innerText + "µ" + table.rows[i].cells[2].innerText + "µ" + table.rows[i].cells[3].innerText + "µ" + table.rows[i].cells[4].innerText + "µ" + table.rows[i].cells[5].innerText + "µ" + table.rows[i].cells[7].innerText + "µ" + table.rows[i].cells[8].innerText + "µ" + table.rows[i].cells[9].innerText + "¥";
        }
        var submitRequest = {

            Flag: "FDINTERESTACCRAUTO",
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasuryConfirmation", submitRequest, FDIntAccrualAuto.SubmitReturn, userdata.token)
    },
...
jQuery(document).ready(function () {
    //FDIntAccrualAuto.loaddata();
    FDIntAccrualAuto.tokenValidate();


    jQuery('#btnConf').on("click", function () {
        FDIntAccrualAuto.submitdata1();
    });
    jQuery('#btn_Excel').on("click", function () {
        FDIntAccrualAuto.exportTableToExcel();
    });
});
