var DocumentationFee = {
    ...
    checkAccess: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/PayratQueries", CheckAccess, DocumentationFee.checkAccessRtn, token)
    },
        ...
        if (response.status === "SUCCESS") {
            ...
            if (response.data != null && response.data.queryResult.QueryResult.length > 0) {
                ...
                if (x == "0") {
                    ...
                    DocumentationFee.loadSelectFinancialInstitution();
                }
    ...
    tokenValidate: function () {
        ...
        _http.post(MFPUBLICACCOUNTSAPI_URL + "api/accounts/UserSession", CheckToken, DocumentationFee.checkAccessToken, userdata.token)
    },
    ...
    loadSelectFinancialInstitution: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebSelectFinancialInstitution, DocumentationFee.FillDebSelectFinancialInstitution, userdata.token);
    ...
    loadSelectFund: function (indata) {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebSelectFund, DocumentationFee.FillDebSelectFund, userdata.token);
    ...
    getFromLoanDtls: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", FromLoanDtls, DocumentationFee.FillTable, userdata.token);

    },
    //FromLoanDtlsResponse: function (Response) {
    //    if (Response.status === "SUCCESS") {

    //        DocumentationFee.FillTable(Response);
    ...
    loadBankAccount: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebBankAccount, DocumentationFee.FillDebBankAccount, userdata.token);
    ...
    loadBankSubAccount: function (indata) {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebBankSubAccount, DocumentationFee.FillDebBankSubAccount, userdata.token);
    ...
    loadContraMainAccountt: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebContraMainAccount, DocumentationFee.FillDebContraMainAccount, userdata.token);
    ...
    loadContraSubAccount: function (indata) {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebContraSubAccount, DocumentationFee.FillDebContraSubAccount, userdata.token);
    ...
    loadContraNo: function (indata) {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebContraNo, DocumentationFee.FillDebContraNo, userdata.token);
    ...
    loadTDSAccount: function () {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebTDSAccount, DocumentationFee.FillDebTDSAccount, userdata.token);
    ...
    loadTDSSubAccount: function (indata) {
        ...
        _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasurySelectQueries", DebTDSSubAccount, DocumentationFee.FillDebTDSSubAccount, userdata.token);
    ...
    checkvalues: function () {
        ...
        if (jQuery("#ddl_fI").val() == "0") {
            ...
            swal("", "Please select an Option - Documentation Fee With IGST/Documentation Fee WithSGST & CGST..!", "error");
            //alert("Please select an Option");
            return false;
        }
        else if (jQuery("#txt_docfee").val() == "") {
            swal("", "Please Enter Documentation Fee...!!!", "error");
            return false;
        }
        else if (jQuery("#ddl_mainAcc").val() == "0" && jQuery("#radNo").prop("checked") == true) {
            swal("", "Please Select Main Bank Account...!!!", "error");
            return false;
        }
        else if (jQuery("#ddl_SubAcc").val() == "0" && jQuery("#radNo").prop("checked") == true) {
            swal("", "Please Select Sub Bank Account...!!!", "error");
            return false;
        }

        else if (jQuery("#ddl_contramainAcc").val() == "0") {
            swal("", "Please Select Main Contra Account...!!!", "error");
            return false;
        }
        //else if (jQuery("#ddl_contraSubAcc").val() == "0") {
        //    swal("", "Please Select Sub Contra Account...!!!", "error");
        //    return false;
            //}
            //else if (jQuery("#txt_pjt").val() == "") {
            //    swal("", "Please Enter Project Code...!!!", "error");
            //    return false;
            //}
            //else if (jQuery("#ddl_contraFIID").val() == "0") {
            //    swal("", "Please Select Contra Number...!!!", "error");
            //    return false;
            //}
        else if (jQuery("#radYes").prop("checked") == false && jQuery("#radNo").prop("checked") == false) {
            swal("", "Please select an Option - Yes/No..!", "error");
            //alert("Please select an Option");
            return false;
        }
        else if (jQuery("#radYes").prop("checked") == true && jQuery("#txt_ifsc").val() == "") {

            swal("", "Please Enter IFSC Code...!!!", "error");
            return false;
        }
            ...
            swal("", "Please Enter Beneficiary Name...!!!", "error");
            return false;
        }
        ...
        //}
            ...
            swal("", "Please select an Option - With TDS/Without TDS..!", "error");
            return false;
        }
        else if (jQuery("#radAppr").prop("checked") == true && jQuery("#ddl_maintds").val() == "0") {
            swal("", "Please Select TDS Main Account...!!!", "error");
            return false;
        }
        else if (jQuery("#radAppr").prop("checked") == true && jQuery("#ddl_tdsSubAcc").val() == "0") {
            swal("", "Please Select TDS Sub Account...!!!", "error");
            return false;
        }
    ...
    submitevalue: function () {
        ...
        if (DocumentationFee.checkvalues()) {
            var DocFeeType, paymode, mainac, subac, tdsmain, tdssub, tdsamnt;

            if (jQuery("#radigst").prop("checked")) {
                DocFeeType = 1;
            }
            if (jQuery("#radsgst").prop("checked")) {
                DocFeeType = 2;
            }
            ...
            var InputData = DocFeeType + "µ" + jQuery("#ddl_Loan").val() + "µ" + jQuery("#txt_docfee").val() + "µ" + mainac + "µ" + subac + "µ" + jQuery("#ddl_contramainAcc").val() + "µ" + jQuery("#ddl_contraSubAcc").val() + "µ" + jQuery("#ddl_contraFIID").val() + "µ" + jQuery("[id*=hdUserId2]").val() + "µ" + jQuery("#txt_ifsc").val() + "µ" + jQuery("#txt_Accnumber").val() + "µ" + jQuery("#txt_Mobnumber").val() + "µ" + jQuery("#txt_Accname").val() + "µ" + paymode + "µ" + jQuery("#txt_pjt").val() + "µ" + tdsmain + "µ" + tdssub + "µ" + tdsamnt;


            var submitRequest = {

                Flag: "DOCUMENTATIONFEE",
                ParVal: InputData,
                typeID: "4",
                userID: userdata.userId,
                branchID: userdata.branchId
            ...
            _http.post(MFPUBLICTREASURYAPI_URL + "api/Treasury/TreasuryConfirmation", submitRequest, DocumentationFee.SubmitReturn, userdata.token)
    ...
    //    _http.post(MFPUBLICCUSTOMERAPI_URL + "api/banks/searchifsc", VIfsc, DocumentationFee.FillIfsc, userdata.token);
    //},
...
jQuery(document).ready(function () {
    ...
    //DocumentationFee.loadSelectFinancialInstitution();
    DocumentationFee.tokenValidate();

    DocumentationFee.loadBankAccount();

    DocumentationFee.loadContraMainAccountt();

    DocumentationFee.loadTDSAccount();
    ...
    jQuery('#ddl_fI').on("change", function () {
        ...
        if (indata == 0) {
            ...
            jQuery("#ddl_Loan").val("");
            jQuery("#ShowtblDiv").hide();
            jQuery("#DivBank").hide();
            jQuery("#maincard").hide();
            jQuery("#tdsdiv").hide();
            jQuery("#ddlsubacBNK").hide();
            jQuery("#ddlcontraIsubID").hide();
            jQuery("#divpjt").hide();
            jQuery("#ddlcontrasub").hide();


            jQuery('#radigst').prop('checked', false);
            jQuery('#radsgst').prop('checked', false);

            jQuery('#radNo').prop('checked', false);
            jQuery('#radYes').prop('checked', false);

            jQuery("#txt_docfee").val("");

            jQuery("#ddl_mainAcc").val(0);

            jQuery("#ddl_contramainAcc").val(0);
            jQuery("#ddl_SubAcc").val("");
            jQuery("#ddl_contraSubAcc").val("");
            jQuery("#txt_pjt").val("");
            jQuery("#ddl_contraFIID").val("");


            jQuery("#txt_ifsc").val("");
            jQuery("#txt_Accnumber").val("");
            jQuery("#txt_Accname").val("");
            jQuery("#txt_Mobnumber").val("");

            jQuery("#txt_tds").val("");
            jQuery("#ddl_tdsSubAcc").val("");
            jQuery("#ddl_maintds").val("");
            jQuery("#ddl_contraFIID").val("");
            ...
            DocumentationFee.loadSelectFund(indata);
        }
    });
    ...
    jQuery('#ddl_Loan').on("change", function () {
        ...
        DocumentationFee.getFromLoanDtls();
    });
    ...
    jQuery('#txt_pjt').on("change", function () {
        ...
        DocumentationFee.loadContraNo(indata);
    });
    ...
    jQuery('#ddl_maintds').on("change", function () {
        ...
        DocumentationFee.loadTDSSubAccount(indata);
    });


    jQuery('#btnConf').on("click", function () {
        DocumentationFee.submitevalue();
    });

    jQuery('#txt_ifsc').on("change", function () {
        DocumentationFee.VerifyIFSC();
    });
    jQuery('#txt_Mobnumber').on("change", function () {
        DocumentationFee.VerifyMob();
    });
    jQuery('#ddl_Loan').on("change", function () {
